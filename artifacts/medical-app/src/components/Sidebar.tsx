...
export default function Sidebar({ isOpen, onClose, userRole = "user" }: SidebarProps) {
  ...
  // Single source of truth for all sidebar content
  const SidebarBody = () => (
    <>
      {/* Header */}
      <div style={{
      ...
      <div style={{
        ...
        {mainLinks.map((link) => <LinkItem key={link.path} {...link} />)}

        <SectionHeader title="Communities" />
        {communityLinks.map((link) => <LinkItem key={link.path} {...link} />)}

        {joinedCommunities.length > 0 && (
          <>
            <SectionHeader title="My Communities" />
            {joinedCommunities.map((comm) => (
              <CommunityLinkItem key={comm.id} iconUrl={comm.icon} label={comm.name} path={`/community/${comm.slug}`} />
            ))}
          </>
        )}
        ...
        {personalLinks.map((link) => <LinkItem key={link.path} {...link} />)}

        <SectionHeader title="Study" />
        {studyLinks.map((link) => <LinkItem key={link.path} {...link} />)}

        {isAdmin && (
          <>
            <SectionHeader title="Moderation" />
            {adminLinks.map((link) => <LinkItem key={link.path} {...link} />)}
          </>
        )}
      </div>
