...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  useEffect(() => {
    async function loadUserData() {
      try {
        ...
        if (creatorData) {
          setCreatedCommunities(creatorData as Community[]);
        }
      } catch (err) {
  ...
  const collapsibleHeaderStyle = (expanded: boolean): React.CSSProperties => ({
    display: "flex",
    alignItems: "center",
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 8px 12px", minHeight: 0 }}>
        <nav style={{ display: "flex", flexDirection: "column", gap: "4px", paddingTop: "4px" }}>
          ...
          {/* Admin Panel */}
          {(profile?.role === "super_admin" || profile?.role === "admin" || profile?.role === "app_admin" || profile?.role === "moderator") && (
          ...
          <div style={{ marginTop: "12px", paddingTop: "12px", borderTop: "1px solid var(--border)" }}>
            ...
            {/* ── Collapsible: Joined Communities ── */}
            <div style={{ marginTop: "4px" }}>
              <button
                onClick={() => setShowJoined((v) => !v)}
                style={collapsibleHeaderStyle(showJoined)}
              >
                ...
                <ChevronDown
                  size={16}
                  style={{
                    transform: showJoined ? "rotate(180deg)" : "rotate(0deg)",
            ...
            {/* ── Collapsible: Created by You ── */}
            {createdCommunities.length > 0 && (
              <div style={{ marginTop: "4px" }}>
                <button
                  onClick={() => setShowCreated((v) => !v)}
                  style={collapsibleHeaderStyle(showCreated)}
                >
                  <span style={{ display: "flex", alignItems: "center", gap: "10px" }}>
