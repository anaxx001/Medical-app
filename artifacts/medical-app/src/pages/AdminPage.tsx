...
/** Role compatibility helper — treats legacy "admin" and new "app_admin" identically */
...
export default function AdminPage() {
  ...
  useEffect(() => {
    async function checkAuth() {
      ...
      if (!profile || !["admin", "app_admin", "moderator", "super_admin"].includes(profile.role)) {
        navigate("/");
        return;
      }
  ...
  async function fetchAllData(role?: string, userId?: string) {
    ...
    try {
      const [
        ...
        supabase
          .from("profiles")
          .select("id, username, full_name, profession, role, created_at, is_suspended, is_muted, is_banned")
          .order("created_at", { ascending: false })
          .range(0, usersPerPage - 1),
        supabase
          .from("posts")
          .select(
            `id, title, content, upvotes, comment_count, created_at, is_pinned, is_announcement, is_reported, is_flagged,
            author:author_id(username, full_name),
            community:community_id(name, icon)`
          )
          .order("created_at", { ascending: false })
          .limit(100),
        (isAppAdmin(effectiveRole) || effectiveRole === "moderator")
          ? supabase
              .from("communities")
              .select("*")
              .order("created_at", { ascending: false })
      ...
      (usersRes.data || []).slice(0, 5).forEach((u: any) => {
        activity.push({
          type: "user",
          message: `New user: ${u.full_name || u.username}`,
      ...
      (postsRes.data || []).slice(0, 5).forEach((p: any) => {
        activity.push({
          type: "post",
          message: `New post: "${p.title?.substring(0, 40) || "Untitled"}..."`,
  ...
  async function updateUserRole(userId: string, newRole: string) {
    ...
    if (currentRole === "super_admin" || isAppAdmin(currentRole)) {
  ...
  async function toggleUserMute(userId: string, currentStatus: boolean) {
    ...
    if (currentRole === "super_admin" || isAppAdmin(currentRole)) {
      await supabase.from("audit_logs").insert({
        admin_id: currentUserId,
        action: !currentStatus ? "Muted user" : "Unmuted user",
        target_type: "user",
        target_id: userId,
      });
    }
    ...
    setUsers((prev) =>
      prev.map((u) => (u.id === userId ? { ...u, is_muted: !currentStatus } : u))
    );
  }
  ...
  async function toggleUserBan(userId: string, currentStatus: boolean) {
    ...
    if (currentRole === "super_admin" || isAppAdmin(currentRole)) {
      await supabase.from("audit_logs").insert({
        admin_id: currentUserId,
        action: !currentStatus ? "Permanently banned user" : "Unbanned user",
        target_type: "user",
        target_id: userId,
      });
    }
  ...
  async function deleteUserAccount(userId: string) {
    if (!confirm("DELETE this user account permanently? All their data will be removed. This cannot be undone.")) return;
    if (!confirm("Are you absolutely sure?")) return;

    await supabase.from("posts").delete().eq("author_id", userId);
    await supabase.from("comments").delete().eq("author_id", userId);
    await supabase.from("reports").delete().eq("reported_by_id", userId);
  ...
  async function togglePostFlag(postId: string, currentStatus: boolean) {
    await supabase.from("posts").update({ is_flagged: !currentStatus }).eq("id", postId);
  ...
  async function toggleCommunityLock(communityId: string, currentStatus: boolean) {
    await supabase.from("communities").update({ is_locked: !currentStatus }).eq("id", communityId);
    setCommunities((prev) =>
      prev.map((c) => (c.id === communityId ? { ...c, is_locked: !currentStatus } : c))
    );
  }
  ...
  const getTabs = () => {
    ...
    if (currentRole === "super_admin") {
      return [
        ...
        { ...communityRequestTab },
        { key: "security_vault", label: "🔐 Security Vault", badge: 0 },
      ];
          ...
          <div>
            ...
            >
              {statCard(
                ...
                "Total Users",
                stats.users,
                Math.round((stats.new_this_week / Math.max(stats.users, 1)) * 100) || 0
              )}
            ...
            {(() => {
              const AUTHORITY_MATRIX: Record<string, { color: string; icon: string; permissions: string[] }> = {
                super_admin: {
                  color: "#E8445A",
                  icon: "🛡️",
                  permissions: ["Global Wipes", "Master Decryption", "Permanent Bans", "Role Overrides"],
                },
                app_admin: {
                  color: "#F5A623",
                  icon: "⚙️",
                  permissions: ["Community Approval", "Keyword Enforcement", "Workspace Locks", "Exam Freeze Controls"],
                },
                moderator: {
                  color: "#2D87C8",
                  icon: "🔎",
                  permissions: ["Mute Members", "Review Reports", "Escalate Incidents", "Lock Communities"],
                },
                exco: {
                  color: "#9B6DFF",
                  icon: "📌",
                  permissions: ["Pin Announcements", "Deploy Syllabus Templates"],
                },
              };
                    ...
                    {Object.entries(AUTHORITY_MATRIX).map(([role, { color, icon, permissions }]) => (
                      <div
                        ...
                        style={{
                          ...
                          padding: "14px",
                          background: `${color}08`,
                          opacity: currentRole === role || (role === "app_admin" && isAppAdmin(currentRole)) ? 1 : 0.6,
                        }}
                      >
                        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
                          <span style={{ fontSize: "18px" }}>{icon}</span>
                          <span style={{ fontSize: "12px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color }}>
                            {role.replace("_", " ")}
                          </span>
                        </div>
          ...
          <div>
                  ...
                  <div key={user.id} style={cardStyle}>
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
                        <button
                          onClick={() => fetchUserPosts(user.id, user.username)}
                          title="View posts"
                          style={{
                            padding: "8px 10px",
                            background: "var(--surface-2)",
                        ...
                        <button
                          onClick={() => toggleUserMute(user.id, user.is_muted || false)}
                          title={user.is_muted ? "Unmute" : "Mute"}
                          style={{
                            padding: "8px 10px",
                            background: user.is_muted ? "#FEF3C7" : "var(--surface-2)",
                            color: user.is_muted ? "#D97706" : "var(--text-muted)",
                            border: "1px solid var(--border)",
                            borderRadius: "var(--radius-sm)",
                            fontSize: "12px",
                            fontWeight: 700,
                        ...
                        {(currentRole === "super_admin" || isAppAdmin(currentRole)) && (
                          <button
                            ...
                            title={user.is_banned ? "Unban" : "Permanently Ban"}
                            style={{
                              padding: "8px 10px",
                              background: user.is_banned ? "#ECFDF5" : "#7C2D12",
                              color: user.is_banned ? "#059669" : "#FED7AA",
                              border: "none",
                              borderRadius: "var(--radius-sm)",
                              fontSize: "12px",
                              fontWeight: 700,
            ...
            {userTotalCount > usersPerPage && (
              ...
              >
                ...
                >
                  Page {userPage} of {Math.ceil(userTotalCount / usersPerPage)}
                </span>
                <button
                  onClick={() => {
                    const maxPage = Math.ceil(userTotalCount / usersPerPage);
                    const newPage = userPage < maxPage ? userPage + 1 : userPage;
                    setUserPage(newPage);
                    fetchUsersPage(newPage);
                  }}
                  disabled={userPage >= Math.ceil(userTotalCount / usersPerPage)}
                  style={{
                    padding: "8px 12px",
                    background:
                      userPage >= Math.ceil(userTotalCount / usersPerPage)
                        ? "var(--border)"
                        : "var(--surface)",
                    color:
                      userPage >= Math.ceil(userTotalCount / usersPerPage)
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                <h2 style={{ margin: 0, fontSize: "18px", fontWeight: 800, color: "var(--text)" }}>
                  📝 Posts by @{userPostsModal.username}
                </h2>
                <button
                  onClick={() => setUserPostsModal({ open: false, userId: "", username: "" })}
                  style={{
              ...
              {userPostsLoading ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                  {userPosts.map((post) => (
                    ...
                    >
                      <p style={{ margin: "0 0 4px 0", fontWeight: 700, fontSize: "14px", color: "var(--text)" }}>
                        {post.title || "Untitled"}
                      </p>
                      <p style={{ margin: "0 0 8px 0", fontSize: "12px", color: "var(--text-muted)" }}>
                        {post.community?.name} • {post.upvotes || 0} upvotes • {post.comment_count || 0} comments
                      </p>
                      <button
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: "8px" }}>
                        <button
                          onClick={() => togglePostFlag(post.id, post.is_flagged || false)}
                          style={{
                            padding: "8px 12px",
                            background: post.is_flagged ? "#FFF8EC" : "var(--surface-2)",
                            color: post.is_flagged ? "#F5A623" : "var(--text-muted)",
                            border: "1px solid var(--border)",
                            borderRadius: "var(--radius-sm)",
                            fontSize: "12px",
                            fontWeight: 700,
                      ...
                      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", justifyContent: "flex-end" }}>
                        {isAppAdmin(currentRole) && (
                          <button
                            ...
                            style={{
                              padding: "8px 12px",
                              background: community.is_official ? "#ECFDF5" : "var(--surface-2)",
                              color: community.is_official ? "#059669" : "var(--text-muted)",
                              border: "1px solid var(--border)",
                              borderRadius: "var(--radius-sm)",
                              fontSize: "12px",
                              fontWeight: 700,
                        ...
                        <button
                          ...
                          style={{
                            padding: "8px 12px",
                            background: community.is_locked ? "#FEF3C7" : "var(--surface-2)",
                            color: community.is_locked ? "#D97706" : "var(--text-muted)",
                            border: "1px solid var(--border)",
                            borderRadius: "var(--radius-sm)",
                            fontSize: "12px",
                            fontWeight: 700,
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", marginBottom: "20px" }}>
              {(["All", "Open", "Resolved", "Escalated"] as const).map((f) => {
                ...
                return (
                  <button
                    key={f}
                    onClick={() => setFilterReportStatus(key)}
                    style={{
                      padding: "7px 16px",
                      borderRadius: "20px",
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                  <thead>
                    <tr style={{ background: "var(--surface-2)", borderBottom: "2px solid var(--border)" }}>
                      {["Community", "Reporter", "Violation", "Preview", "Status", "Date", "Actions"].map((col) => (
                        <th
                          key={col}
                          style={{
                            padding: "10px 14px",
                    ...
                    {filteredReports.length === 0 ? (
                      ...
                      filteredReports.map((report) => {
                        const violationColor: Record<string, string> = {
                          cyberbullying: "#E8445A",
                          exam_leak: "#F5A623",
                          harassment: "#E8445A",
                          spam: "#9B6DFF",
                          misinformation: "#2D87C8",
                        };
                        ...
                        return (
                          ...
                          >
                            <td style={{ padding: "10px 14px", color: "var(--text)", fontWeight: 600 }}>
                              {report.originating_community_id
                                ? `r/${report.originating_community_id}`
                                : "—"}
                            </td>
                            ...
                            <td style={{ padding: "10px 14px", color: "var(--text-muted)", maxWidth: "200px" }}>
                              <span style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                {report.content_preview || report.content?.substring(0, 80) || "—"}
                              </span>
                            </td>
                            <td style={{ padding: "10px 14px" }}>
                              <span
            ...
            {!vaultUnlocked ? (
              ...
              <div style={{ maxWidth: "480px", margin: "40px auto" }}>
                <div style={{ ...cardStyle, textAlign: "center" }}>
                  ...
                  <h2 style={{ fontSize: "20px", fontWeight: 800, color: "var(--text)", margin: "0 0 6px 0" }}>
                    Security Vault
                  </h2>
                  <p style={{ fontSize: "13px", color: "var(--text-muted)", margin: "0 0 24px 0" }}>
                    Access to encrypted message audits is restricted. Enter your master password and log your reason.
                  </p>
                  ...
                  <div style={{ marginBottom: "14px", textAlign: "left" }}>
                    <label style={{ fontSize: "12px", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em", display: "block", marginBottom: "6px" }}>
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      />
                      <button
                        onClick={() => setVaultShowPassword((v) => !v)}
                        style={{
                  ...
                  <div style={{ marginBottom: "20px", textAlign: "left" }}>
                    <label style={{ fontSize: "12px", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em", display: "block", marginBottom: "6px" }}>
                      Justification Log Entry
                    </label>
                    <textarea
                      rows={3}
                      value={vaultJustification}
                      onChange={(e) => setVaultJustification(e.target.value)}
                      placeholder="Why are you accessing the security vault? (min. 10 characters)"
                      style={{
                    ...
                    />
                    <div style={{ fontSize: "11px", color: vaultJustification.length >= 10 ? "#059669" : "var(--text-muted)", marginTop: "4px", textAlign: "right" }}>
                      {vaultJustification.length}/10 min
                    </div>
                  </div>

                  <button
                    disabled={vaultLoading || vaultPassword.length === 0 || vaultJustification.length < 10}
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (!res.ok || !json.success) {
                          setVaultError(json.message || "Incorrect password.");
                        } else {
                          // Log vault access
                          await supabase.from("vault_access_logs").insert({
                            admin_id: currentUserId,
                            justification: vaultJustification,
                            accessed_at: new Date().toISOString(),
                          });
                          // Log to audit_logs too
                          await supabase.from("audit_logs").insert({
                            admin_id: currentUserId,
                            action: "vault_access",
                            details: { justification: vaultJustification },
                            created_at: new Date().toISOString(),
                          });
                          // Fetch audits
                          setVaultAuditsLoading(true);
                          const { data } = await supabase
                            .from("encrypted_message_audits")
                            .select("*")
                            .order("created_at", { ascending: false })
                            .limit(200);
                          setVaultAudits(data ?? []);
                        ...
                        setVaultError("Network error — could not reach authentication server.");
                      } finally {
                    ...
                    style={{
                      ...
                      background: vaultLoading || vaultPassword.length === 0 || vaultJustification.length < 10
                        ? "var(--border)" : "#E8445A",
                      color: "#fff", border: "none", borderRadius: "8px",
                      fontWeight: 800, fontSize: "14px", cursor:
                        vaultLoading || vaultPassword.length === 0 || vaultJustification.length < 10
                          ? "not-allowed" : "pointer",
                    }}
                  >
              ...
              <div>
                ...
                <div style={{ ...cardStyle, padding: 0, overflow: "hidden" }}>
                  {vaultAuditsLoading ? (
                    <p style={{ padding: "32px", textAlign: "center", color: "var(--text-muted)" }}>Loading audits…</p>
                  ) : vaultAudits.length === 0 ? (
                          ...
                          {vaultAudits.map((audit) => (
                            <tr key={audit.id} style={{ borderBottom: "1px solid var(--border)" }}>
                              <td style={{ padding: "10px 14px", fontWeight: 600, color: "var(--text)" }}>
                                {audit.sender_id ? (
                                  <span style={{
                                    padding: "2px 8px", background: "#F5F0FF", color: "#9B6DFF",
                              ...
                              <td style={{ padding: "10px 14px", color: "var(--text-muted)" }}>
                                {audit.recipient_id ? `@${audit.recipient_username ?? audit.recipient_id}` : "—"}
                              </td>
                              <td style={{ padding: "10px 14px" }}>
                                <span style={{
                                  padding: "2px 8px", background: "#FFF8EC", color: "#F5A623",
                                  borderRadius: "4px", fontWeight: 700, fontSize: "12px",
                                }}>
                                  {audit.flagged_keyword ?? "—"}
                                </span>
                              </td>
                                ...
                                {audit.is_anonymous && (
                                  <button
                                    onClick={async () => {
                                      ...
                                      await supabase.from("identity_reveal_logs").insert({
                                        admin_id: currentUserId,
                                        target_message: audit.id,
                                        reason: reason.trim(),
                                        created_at: new Date().toISOString(),
                                      });
                                      await supabase.from("audit_logs").insert({
                                        ...
                                        details: { target_message: audit.id, reason: reason.trim() },
                                        created_at: new Date().toISOString(),
                                      });
