...
const NIGERIAN_UNIVERSITIES = [
  ...
  { slug: "unn", name: "University of Nigeria" },
  { slug: "abu", name: "Ahmadu Bello University" },
  { slug: "lasu", name: "Lagos State University" },
  { slug: "unilorin", name: "University of Ilorin" },
  { slug: "global", name: "All Universities (Global)" },
];
                  ...
                  {recentActivity.length === 0 ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                      {recentActivity.map((a, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px", borderRadius: "10px", background: "rgba(0,0,0,0.03)" }}>
                          <div style={{
              ...
              <div>
                      ...
                      {filteredUsers.map((u) => (
                        <tr key={u.id} style={{ borderBottom: "1px solid var(--border)" }}>
                          <td style={{ padding: "12px" }}>
                            ...
                            <div style={{ fontSize: "12px", color: "var(--text-muted)" }}>{u.full_name || "—"}</div>
                          </td>
                          <td style={{ padding: "12px" }}>
                            <span style={{
                          ...
                          <td style={{ padding: "12px", color: "var(--text-muted)" }}>{u.profession || "—"}</td>
                          <td style={{ padding: "12px" }}>
                          ...
                          <td style={{ padding: "12px", textAlign: "right" }}>
                            <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end" }}>
                              <button onClick={() => fetchUserPosts(u.id, u.username)} title="View Posts" style={{ padding: "6px", borderRadius: "6px", border: "none", background: "rgba(0,0,0,0.03)", cursor: "pointer" }}>
                                <Eye size={14} />
                              </button>
                              {(isAdmin || isSuper) && (
                                <>
                                  <button onClick={() => toggleUserMute(u.id, !!u.is_muted)} title={u.is_muted ? "Unmute" : "Mute"} style={{ padding: "6px", borderRadius: "6px", border: "none", background: u.is_muted ? "#EBF5FF" : "rgba(0,0,0,0.03)", cursor: "pointer", color: u.is_muted ? "#2D87C8" : "inherit" }}>
                                    {u.is_muted ? <Volume2 size={14} /> : <VolumeX size={14} />}
                                  </button>
                                  <button onClick={() => toggleUserSuspend(u.id, !!u.is_suspended)} title={u.is_suspended ? "Unsuspend" : "Suspend"} style={{ padding: "6px", borderRadius: "6px", border: "none", background: u.is_suspended ? "#FFF8EC" : "rgba(0,0,0,0.03)", cursor: "pointer", color: u.is_suspended ? "#F5A623" : "inherit" }}>
                                    {u.is_suspended ? <Unlock size={14} /> : <Lock size={14} />}
                                  </button>
                                  <button onClick={() => toggleUserBan(u.id, !!u.is_banned)} title={u.is_banned ? "Unban" : "Ban"} style={{ padding: "6px", borderRadius: "6px", border: "none", background: u.is_banned ? "#EDFFF5" : "rgba(0,0,0,0.03)", cursor: "pointer", color: u.is_banned ? "#3DBE7A" : "inherit" }}>
                ...
                {userTotalCount > usersPerPage && (
                  <div style={{ display: "flex", justifyContent: "center", gap: "8px", marginTop: "16px" }}>
                    <button onClick={() => { setUserPage((p) => Math.max(1, p - 1)); fetchUsersPage(Math.max(1, userPage - 1)); }} disabled={userPage === 1} style={{ padding: "8px 16px", borderRadius: "8px", border: "none", background: "rgba(0,0,0,0.03)", cursor: userPage === 1 ? "not-allowed" : "pointer" }}>Previous</button>
                    <span style={{ padding: "8px 16px" }}>Page {userPage} of {Math.ceil(userTotalCount / usersPerPage)}</span>
                    <button onClick={() => { setUserPage((p) => p + 1); fetchUsersPage(userPage + 1); }} disabled={userPage >= Math.ceil(userTotalCount / usersPerPage)} style={{ padding: "8px 16px", borderRadius: "8px", border: "none", background: "rgba(0,0,0,0.03)", cursor: userPage >= Math.ceil(userTotalCount / usersPerPage) ? "not-allowed" : "pointer" }}>Next</button>
                  </div>
                )}
              </div>
            )}
                  ...
                  {filteredPosts.map((p) => (
                    <div key={p.id} style={{ padding: "16px", borderRadius: "12px", background: "var(--surface)", border: "1px solid var(--border)" }}>
                      ...
                      <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                        <span style={{ fontSize: "13px", color: "var(--text-muted)" }}>▲ {p.upvotes} · 💬 {p.comment_count}</span>
                        <div style={{ flex: 1 }} />
                        {(isAdmin || isSuper) && (
                          <>
                            <button onClick={() => handleTogglePin(p.id, p.is_pinned)} style={{ padding: "6px 12px", borderRadius: "6px", border: "none", background: "rgba(0,0,0,0.03)", cursor: "pointer", fontSize: "13px" }}>
                              {p.is_pinned ? "Unpin" : "Pin"}
                            </button>
                            <button onClick={() => handleToggleAnnouncement(p.id, p.is_announcement)} style={{ padding: "6px 12px", borderRadius: "6px", border: "none", background: "rgba(0,0,0,0.03)", cursor: "pointer", fontSize: "13px" }}>
                  ...
                  {filteredCommunities.map((c) => (
                    <div key={c.id} style={{ padding: "16px", borderRadius: "12px", background: "var(--surface)", border: "1px solid var(--border)" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "12px" }}>
                        {c.icon ? <img src={c.icon} alt="" style={{ width: "40px", height: "40px", borderRadius: "10px", objectFit: "cover" }} /> : <div style={{ width: "40px", height: "40px", borderRadius: "10px", background: "rgba(0,0,0,0.03)", display: "flex", alignItems: "center", justifyContent: "center" }}><Lock size={18} style={{ color: "var(--text-muted)" }} /></div>}
                        <div>
                          <div style={{ fontWeight: 600 }}>{c.name}</div>
                          <div style={{ fontSize: "12px", color: "var(--text-muted)" }}>/{c.slug} · {c.member_count} members · {c.post_count} posts</div>
                        </div>
                      </div>
                  ...
                  {filteredReports.map((r) => (
                    <div key={r.id} style={{ padding: "16px", borderRadius: "12px", background: "var(--surface)", border: "1px solid var(--border)" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                          ...
                          <span style={{
                            ...
                            background: r.status === "resolved" ? "#EDFFF5" : r.status === "dismissed" ? "rgba(0,0,0,0.03)" : "#FFF0F2",
                            color: r.status === "resolved" ? "#3DBE7A" : r.status === "dismissed" ? "var(--text-muted)" : "#E8445A",
                          }}>
                            {r.status || "pending"}
                          </span>
                        </div>
                      ...
                      <div style={{ marginBottom: "8px" }}>
                        ...
                        {r.violation_category && <div style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "4px" }}>Category: {r.violation_category}</div>}
                        {r.content_preview && <div style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "4px", fontStyle: "italic" }}>"{r.content_preview}"</div>}
                      </div>
                      <div style={{ display: "flex", gap: "8px" }}>
                        <span style={{ fontSize: "12px", color: "var(--text-muted)" }}>Reported by: {r.reporter?.username || "Unknown"}</span>
                        <div style={{ flex: 1 }} />
                        {(isAdmin || isSuper) && r.status !== "resolved" && r.status !== "dismissed" && (
                          <>
                            ...
                            <button onClick={() => handleDismissReport(r.id)} style={{ padding: "6px 12px", borderRadius: "6px", border: "none", background: "rgba(0,0,0,0.03)", cursor: "pointer", fontSize: "13px" }}>
                    ...
                    <tbody>
                      {filteredAuditLog.map((log) => (
                        <tr key={log.id} style={{ borderBottom: "1px solid var(--border)" }}>
                          ...
                          <td style={{ padding: "12px", fontSize: "13px", color: "var(--text-muted)" }}>{log.target_type}:{log.target_id.substring(0, 8)}...</td>
                          <td style={{ padding: "12px", fontSize: "13px", color: "var(--text-muted)" }}>
                            {log.details ? JSON.stringify(log.details).substring(0, 50) : "—"}
                          </td>
                          <td style={{ padding: "12px", fontSize: "13px", color: "var(--text-muted)", whiteSpace: "nowrap" }}>
                        ...
                        <div style={{ display: "flex", gap: "8px" }}>
                          {rejectingId === req.id ? (
                            <div style={{ flex: 1, display: "flex", gap: "8px" }}>
                              ...
                              <button onClick={() => { setRejectingId(null); setRejectionReason(""); }} style={{ padding: "8px 16px", borderRadius: "8px", border: "none", background: "rgba(0,0,0,0.03)", cursor: "pointer" }}>Cancel</button>
                            </div>
                          ) : (
                            <>
              ...
              <div>
                {!vaultUnlocked ? (
                  <div style={{ maxWidth: "400px", margin: "0 auto", padding: "40px", borderRadius: "16px", background: "var(--surface)", border: "1px solid var(--border)", textAlign: "center" }}>
                    ...
                    <p style={{ color: "var(--text-muted)", marginBottom: "24px" }}>This area contains sensitive actions. Authentication required.</p>
                    <input
                      type={vaultShowPassword ? "text" : "password"}
                      placeholder="Vault password"
                      value={vaultPassword}
                      onChange={(e) => setVaultPassword(e.target.value)}
                      style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1px solid var(--border)", background: "rgba(0,0,0,0.03)", color: "var(--text)", marginBottom: "12px" }}
                    />
                    <input
                      type="text"
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "24px" }}>
                      ...
                      <button onClick={() => setVaultUnlocked(false)} style={{ marginLeft: "auto", padding: "8px 16px", borderRadius: "8px", border: "none", background: "rgba(0,0,0,0.03)", cursor: "pointer" }}>
                        Lock Vault
                      </button>
                    </div>
                        ...
                        {vaultAudits.map((a) => (
                          <div key={a.id} style={{ padding: "16px", borderRadius: "12px", background: "var(--surface)", border: "1px solid var(--border)", borderLeft: "4px solid #E8445A" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                              ...
                              <span style={{ fontSize: "12px", color: "var(--text-muted)" }}>{new Date(a.created_at).toLocaleString()}</span>
                            </div>
                            <div style={{ fontSize: "13px", color: "var(--text-muted)" }}>
                              By: {a.admin?.username || "Unknown"} · Target: {a.target_type}:{a.target_id}
                            </div>
                            {a.details?.justification && (
                              <div style={{ marginTop: "8px", padding: "8px", borderRadius: "6px", background: "rgba(0,0,0,0.03)", fontSize: "13px" }}>
              ...
              <div>
                        ...
                        <div style={{
                          ...
                          {moderatorSearchResults.map(u => (
                            ...
                            >
                              ...
                              <div style={{ fontSize: "12px", color: "var(--text-muted)" }}>@{u.username} · {u.role}</div>
                            </div>
                          ))}
                        </div>
                      )}
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                            {["news:create", "news:edit_own", "news:edit_any", "news:pin", "news:publish"].map(p => (
                              <label key={p} style={{
                                ...
                                border: "1px solid var(--border)",
                                cursor: "pointer",
                                background: modPermissions.includes(p) ? "rgba(13, 148, 136, 0.1)" : "rgba(0,0,0,0.03)",
                              }}>
                                <input
                                  type="checkbox"
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                            {NIGERIAN_UNIVERSITIES.map(u => (
                              <label key={u.slug} style={{
                                ...
                                border: "1px solid var(--border)",
                                cursor: "pointer",
                                background: modScope.includes(u.slug) ? "rgba(13, 148, 136, 0.1)" : "rgba(0,0,0,0.03)",
                              }}>
                                <input
                                  type="checkbox"
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                    ...
                    <div key={mod.user_id} style={{
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
                        <div>
                          ...
                          <div style={{ fontSize: "13px", color: "var(--text-muted)" }}>@{mod.user?.username} · {mod.user?.role}</div>
                        </div>
                        <button
                  ...
                  {moderators.filter(m => m.role === "moderator").length === 0 && (
                    <p style={{ color: "var(--text-muted)", textAlign: "center", padding: "40px" }}>No moderators assigned yet.</p>
                  )}
                </div>
              </div>
            )}
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "24px" }} onClick={() => setUserPostsModal({ open: false, userId: "", username: "" })}>
            <div style={{ background: "var(--surface)", borderRadius: "16px", maxWidth: "700px", width: "100%", maxHeight: "80vh", overflow: "auto", padding: "24px" }} onClick={(e) => e.stopPropagation()}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                ...
                <button onClick={() => setUserPostsModal({ open: false, userId: "", username: "" })} style={{ padding: "8px", borderRadius: "8px", border: "none", background: "rgba(0,0,0,0.03)", cursor: "pointer" }}><X size={18} /></button>
              </div>
              {userPostsLoading ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                  {userPosts.map((p) => (
                    <div key={p.id} style={{ padding: "12px", borderRadius: "10px", background: "rgba(0,0,0,0.03)" }}>
                      <div style={{ fontWeight: 600, marginBottom: "4px" }}>{p.title || "Untitled"}</div>
                      <div style={{ fontSize: "13px", color: "var(--text-muted)", marginBottom: "8px" }}>{p.content.substring(0, 150)}{p.content.length > 150 ? "..." : ""}</div>
                      <div style={{ fontSize: "12px", color: "var(--text-muted)" }}>▲ {p.upvotes} · 💬 {p.comment_count} · {new Date(p.created_at).toLocaleDateString()}</div>
                    </div>
                  ))}
                </div>
              )}
