  ...
  async function fetchAll() {
    ...
    try {
      const [
        ...
        supabase.from("posts").select(`id, title, is_announcement, is_pinned, upvotes, created_at, author:profiles!author_id(username), community:communities!community_id(name, icon)`).order("created_at", { ascending: false }).limit(50),
      ]);
    ...
    <AppShell>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: "6px", marginBottom: "24px" }}>
          {tabs.map(({ key, label }) => (
            <button key={key} onClick={() => setTab(key)} style={{ padding: "8px 18px", borderRadius: "99px", border: "none", background: tab === key ? "linear-gradient(135deg, #E8445A, #F5A623)" : "var(--surface)", color: tab === key ? "white" : "var(--text-muted)", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "13px", cursor: "pointer" }}>
              {label}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            {tab === "users" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                <input type="text" placeholder="Search users..." value={search} onChange={e => setSearch(e.target.value)} style={{ width: "100%", padding: "11px 14px", borderRadius: "var(--radius-sm)", border: "1px solid var(--border)", background: "var(--surface)", fontFamily: "var(--font-body)", fontSize: "14px", color: "var(--text)", outline: "none", boxSizing: "border-box" }} />
                  ...
                  {filteredUsers.map((u, i) => (
                    <div key={u.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: i < filteredUsers.length - 1 ? "1px solid var(--border)" : "none", gap: "12px", flexWrap: "wrap" }}>
                      ...
                      <div style={{ position: "relative" }}>
                        <select value={u.role} onChange={e => updateRole(u.id, e.target.value)} disabled={updatingId === u.id} style={{ padding: "6px 28px 6px 10px", borderRadius: "99px", border: `1px solid ${roleColors[u.role]?.color || "var(--border)"}`, background: roleColors[u.role]?.bg || "var(--surface-2)", color: roleColors[u.role]?.color || "var(--text)", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "12px", cursor: "pointer", appearance: "none", outline: "none" }}>
                  ...
                  <div key={post.id} style={{ background: "var(--surface)", borderRadius: "var(--radius)", border: "1px solid var(--border)", padding: "14px 16px", boxShadow: "var(--shadow)" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "10px", flexWrap: "wrap" }}>
                      ...
                      <div style={{ display: "flex", gap: "6px", flexShrink: 0, flexWrap: "wrap" }}>
                        <button onClick={() => toggleAnnouncement(post.id, post.is_announcement)} style={{ padding: "5px 10px", borderRadius: "99px", border: "none", background: post.is_announcement ? "#EBF5FF" : "var(--surface-2)", color: post.is_announcement ? "var(--blue)" : "var(--text-muted)", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "11.5px", cursor: "pointer" }}>
                          📢 {post.is_announcement ? "Unannounce" : "Announce"}
                        </button>
                        <button onClick={() => togglePin(post.id, post.is_pinned)} style={{ padding: "5px 10px", borderRadius: "99px", border: "none", background: post.is_pinned ? "#EDFFF5" : "var(--surface-2)", color: post.is_pinned ? "var(--green)" : "var(--text-muted)", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "11.5px", cursor: "pointer" }}>
                          📌 {post.is_pinned ? "Unpin" : "Pin"}
                        </button>
                        <button onClick={() => deletePost(post.id)} style={{ padding: "5px 10px", borderRadius: "99px", border: "none", background: "#FFF0F2", color: "#E8445A", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "11.5px", cursor: "pointer", display: "flex", alignItems: "center", gap: "4px" }}>
