...
export default function EditNewsPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function loadNews() {
      ...
      if (data) {
        ...
        setSimplified(data.simplified_content || "");
        setCategory(data.category || "Nigeria pediatric policy");
      }
      setLoading(false);
    }
    loadNews();
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || "Failed to update news bulletin.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)" }}>
        <p style={{ color: "var(--text-muted)", fontSize: "14px" }}>Retrieving news dataset...</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: "720px", margin: "0 auto", padding: "24px 16px 80px", fontFamily: "var(--font-body)", color: "var(--text)" }}>
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "20px", padding: "28px" }}>
        <h1 style={{ fontFamily: "var(--font-display)", fontSize: "24px", fontWeight: 800, margin: "0 0 6px" }}>Edit News Bulletin</h1>
        <p style={{ fontSize: "13px", color: "var(--text-muted)", marginBottom: "24px" }}>Correct parameters or expand simplified study guidelines.</p>
        ...
        <form onSubmit={handleUpdate} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <button 
            type="submit"
            disabled={saving}
            style={{ width: "100%", padding: "13px", borderRadius: "10px", border: "none", background: "var(--gradient)", color: "white", fontFamily: "var(--font-display)", fontWeight: 750, fontSize: "14px", cursor: saving ? "not-allowed" : "pointer" }}
          >
            {saving ? "Updating bullet..." : "Save News Changes ✨"}
          </button>
        </form>
      </div>
