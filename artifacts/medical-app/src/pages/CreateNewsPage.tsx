...
export default function CreateNewsPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (profile?.university) {
        setForm(prev => ({
          ...prev,
          universityTags: [profile.university],
        }));
      }
  ...
  useEffect(() => {
    if (form.body && !form.excerpt) {
      const autoExcerpt = form.body.slice(0, 160).replace(/\n/g, " ");
      setForm(prev => ({ ...prev, excerpt: autoExcerpt + (form.body.length > 160 ? "..." : "") }));
    }
  }, [form.body]);

  useEffect(() => {
    if (form.mediaUrl && (form.type === "video" || form.type === "link")) {
      setPreviewMedia(form.mediaUrl);
    } else {
      setPreviewMedia(null);
    }
  }, [form.mediaUrl, form.type]);
  ...
  const validate = (): boolean => {
    ...
    if (form.type === "video" && !form.mediaUrl.trim()) {
      newErrors.mediaUrl = "Video URL is required for video posts";
    }
    if (form.type === "link" && !form.mediaUrl.trim()) {
      newErrors.mediaUrl = "Link URL is required for link posts";
    }
  ...
  const canCreate = userPermissions.includes("news:create") || 
    ["super_admin", "app_admin", "moderator"].includes(userRole || "");
        ...
        <div style={{ maxWidth: "720px", margin: "0 auto", padding: "40px 0" }}>
          <div style={{
            ...
            <p style={{ margin: 0, color: "#EF4444", fontSize: "14px", fontWeight: 500 }}>
              You don't have permission to create news posts.
            </p>
          </div>
          <button
            onClick={() => navigate("/news")}
            style={{ marginTop: "16px", padding: "8px 16px", borderRadius: "var(--radius-sm)", border: "1px solid var(--border)", background: "var(--surface)", cursor: "pointer" }}
          >
            ← Back to News
          </button>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          {(form.type === "video" || form.type === "link") && (
            <div>
              ...
              {previewMedia && form.type === "video" && getYouTubeThumbnail(previewMedia) && (
                <div style={{ marginTop: "10px", borderRadius: "var(--radius-sm)", overflow: "hidden", width: "200px", height: "120px" }}>
                  <img src={getYouTubeThumbnail(previewMedia)!} alt="Preview" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                </div>
              )}
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end", paddingTop: "8px", borderTop: "1px solid var(--border)" }}>
            ...
            >
              {saving && form.status === "draft" && <Loader2 size={16} style={{ animation: "spin 1s linear infinite" }} />}
              Save Draft
            </button>
            ...
            {canPublish && (
              ...
              >
                {saving && form.status === "published" && <Loader2 size={16} style={{ animation: "spin 1s linear infinite" }} />}
                Publish Now
              </button>
            )}
          </div>
          ...
          {!canPublish && (
            <p style={{ fontSize: "12px", color: "var(--text-muted)", textAlign: "right", margin: 0 }}>
              Your post will be saved as a draft for admin review.
            </p>
          )}
        </div>
      </div>
