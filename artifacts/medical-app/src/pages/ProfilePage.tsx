...
export default function ProfilePage() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      const uniqueDays = Array.from(
        new Set((activityDays || []).map((a: any) => a.created_at.split("T")[0]))
      ).sort().reverse();
      let streak = 0;
  ...
  async function handleFollowToggle() {
    ...
    } else if (isPrivate && currentUserId !== profile.id) {
    ...
    <AppShell>
                  ...
                  <>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: "99px",
                        border: "none",
                        background: isFollowing ? "rgba(0,0,0,0.03)" : followRequestPending ? "rgba(0,0,0,0.06)" : "#1ABC9C",
                        color: isFollowing ? "var(--text)" : followRequestPending ? "var(--text-muted)" : "#fff",
                        fontFamily: "var(--font-display)",
                        fontWeight: 600,
                        fontSize: "12px",
                        cursor: "pointer",
      ...
      {showEditModal && (
        <Modal onClose={() => setShowEditModal(false)} title="Edit Profile">
          ...
          {(editModalTab === "profile" || editModalTab === "privacy") && (
          <div
            style={{
