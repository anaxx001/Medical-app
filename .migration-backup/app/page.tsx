...
export default function HomePage() {
  ...
  // Fetch posts safely without breaking state tracking
  useEffect(() => {
  ...
  // Filter local posts cleanly if user searches
  ...
  return (
    <AppShell>
      <div style={{ maxWidth:"900px", margin:"0 auto", padding: "0 16px 24px" }}>
        ...
        {/* ADDED: Reddit-style Pill Search Bar at the very top */}
        <div style={{ 
          display: "flex", 
          alignItems: "center", 
          gap: "12px", 
            ...
            {announcements.length > 0 && (
              <div style={{ background: "linear-gradient(135deg, rgba(45,135,200,0.08), rgba(155,109,255,0.04))", border: "1px solid var(--border)", borderRadius: "var(--radius)", overflow: "hidden", marginBottom: "16px", boxShadow: "var(--shadow)" }}>
                <div onClick={() => setIsAnnouncementsExpanded(!isAnnouncementsExpanded)} style={{ padding: "14px 16px", display: "flex", gap: "12px", alignItems: "center", cursor: "pointer", userSelect: "none" }}>
                  <div style={{ background: "var(--blue)", borderRadius: "50%", padding: "6px", display: "flex", alignItems: "center", justifyCon: "center", color: "white" }}><Megaphone size={16} /></div>
                  <div style={{ flex: 1 }}>
                ...
                {isAnnouncementsExpanded && (
                  <div style={{ borderTop: "1px solid var(--border)", background: "rgba(0,0,0,0.01)", maxHeight: "260px", overflowY: "auto", padding: "12px 16px 16px 16px" }}>
              ...
              <div key={label} style={{ background:"var(--surface)", borderRadius:"var(--radius)", border:"1px solid var(--border)", overflow:"hidden" }}>
                ...
                <div style={{ padding:"6px 8px" }}>
                  {communities.map(({ name, slug, icon }) => (
                    <Link key={slug} href={`/community/${slug}`} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"7px 8px", borderRadius:"8px", textDecoration:"none" }}>
